...
- **Arqueo del día**: además de "Caja real (conteo físico)" y "Cheques / valores", cada día tiene **"Créditos a cobrar"** (`f_cred`/`d.cred`) — mismo patrón, opcional, solo informativo (no entra en el cálculo de `dif`, igual que `chq`). Se edita en el editor de día y se ve en "Control de caja" del modal de detalle.
